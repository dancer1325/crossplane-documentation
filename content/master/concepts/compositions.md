...
* == 
  ...
  * [Enabling Composite Resources](#enabling-composite-resources)
    * -- to -- use this Composition template
  * OPTIONALLY, support
    * [Modifying & patching](#changing-resource-fields) resource settings
    * [Storing connection details](#storing-connection-details) & secrets /
      * -- generated by the -- managed resources
    * [Composition Functions](#use-composition-functions) to -- via custom programs, -- template
      resources
    * create [custom check ABOUT when a resource is ready](#resource-readiness-checks)
 
###  Resource templates

* Composition's spec's `resources`
  * 👀== set of things / Composite Resource creates 👀
  * `[].name`
    * == name / 
      * identify resource | Composition
      * ❌-- NOT related to -- external resource ❌
  * [here](./composite-resources.md)
  
* _Example:_ Composition define a template / | SAME time, create
  * virtual machine
...
* `spec.resources[].base`
  * == create a standalone [managed resource](./managed-resources)
  * _Example:_ [Upbound's Provider AWS](https://marketplace.upbound.io/providers/upbound/provider-aws/v0.35.0)
...
* `crossplane.io/external-name`
  ...
  * -- impact the -- external resource name

#### Template a ProviderConfig

* `ProviderConfig`
  * uses
    * provide unique credentials / EACH deployment
...
* Compositions vs Composite Resources
  * define MORE complicated templates
  * use cases
    * Composition / uses OTHER Compositions 

* _Example:_ Composition / create a standard set of networking resources
  * BOTH Compositions -- must have -- corresponding XRDs

* _Examples:_ [here](https://github.com/upbound/platform-ref-aws)
  * _Example1:_ networking Composition / resources -- required to create a -- new AWS virtual network
    ...
    * `spec.compositeTypeRef`
      * `.apiVersion`
      * `.kind`
    * [Enabling a Composite Resource](#enabling-composite-resources)
      * `spec.compositeTypeRef`
  * _Example2:_ Composition / define OTHER resources | AWS Elastic Kubernetes Cluster
    * -- referring the -- PREVIOUS XNetwork
...
* Composition's resources can cross-reference -- by matching a label OR controller reference -- OTHER resources 
